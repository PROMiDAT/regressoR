...
mod_cv_knn_server <- function(input, output, session, updateData, codedioma){
  ...
  M <- rv(MCs.knn = NULL, grafico = NULL, er = NULL, ea = NULL, corr = NULL, times = 0)
  ...
  output$e_knn_ind  <-  renderEcharts4r({
    ...
    tryCatch({
      ...
      if(!is.null(grafico)){
        ...
        switch (type,
                "barras" = return( resumen.barras(grafico, labels = c(label,"Kernel" ))),
                "error" = return( resumen.error(grafico, labels = c(label, "Kernel", tr("maximo", idioma),tr("minimo", idioma)))),
