...
indices.generales <- function(real, prediccion) {
  ...
  return(list(Error.Cuadratico = MSE,
              ...
              Error.Absoluto = MAE,
              Error.Relativo = RE,
              Correlacion = as.numeric(cor(real, prediccion))))
}
...
#   err <- as.data.frame(indices)
#   colnames(err) <- c(tr("MSE"), tr("RMSE"), tr("MAE"), tr("ER"), tr("correlacion"))
...
#   return(paste0("real <- datos.prueba$",variable.p,"\n",
#                 "prediccion <- prediccion.dt\n",
#                 "MC.dt <<- table(real, prediccion)"))
# }
